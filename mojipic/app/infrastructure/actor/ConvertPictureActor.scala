...
class ConvertPictureActor @Inject()(
  ...
  private[this] def convert(pictureProperty: PictureProperty, picturePropertyRepository: PicturePropertyRepository): Unit = {
    ...
    val convertedPictureProperty: PictureProperty = Try {
      ...
      case Success(_) => PictureProperty(pictureProperty.id,
        pictureProperty.value
          .copy(status = PictureProperty.Status.Success, convertedFilepath = Some(filepath.toString)))
      case Failure(t) =>
